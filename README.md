...
### ✨ 문제 해결 방법
- ✅ **웹메일 인증**으로 강원대 재학생만 이용 가능 → 목적지·시간 조율의 번거로움 해소
...
4. 계산된 금액을 각 멤버의 `totalSavedAmount` 필드에 누적 저장합니다.
...
4. 발송된 방은 `departureNotificationSent = true` 로 변경하여 중복 발송을 방지합니다.
...
**전제 조건:** 본 앱은 *강원대학교 재학생*만 사용 가능합니다.
...
### 카풀 참여 (파티원)
1. `카풀 참여하기` 버튼 선택
2. 원하는 시간과 장소를 필터링하여 참여할 파티 검색
3. 참여 완료

---

## 🔧 트러블슈팅
> 성능 향상 경험을 중심으로 작성 예정 
📌 **[공란]**
...
| 이름 | GitHub | 역할 |
|------|---------|------|
| 권순호 | [공란] | 인증/인가, 카풀 도메인, FCM, 채팅, DB 서버 관리 |
| 이진호 | [공란] | 회원 도메인, FCM, 채팅, 장소 검색, EC2 서버 관리 |
...
## 📑 백엔드 API 명세서
📌 **[공란]**
> API 명세서는 **프로젝트 Wiki**에서 자세히 확인할 수 있습니다.
